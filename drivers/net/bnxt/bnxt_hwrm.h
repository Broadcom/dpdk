...
int bnxt_hwrm_cfa_vlan_antispoof_cfg(struct bnxt *bp,
				     uint16_t fid,
				     uint16_t vlan_count,
...
int bnxt_hwrm_exec_fwd_resp(struct bnxt *bp,
			    uint16_t target_id,
			    void *encaped,
			    size_t ec_size);

int bnxt_hwrm_reject_fwd_resp(struct bnxt *bp,
			      uint16_t target_id,
			      void *encaped,
			      size_t ec_size);
...
int bnxt_hwrm_func_qstats_tx_drop(struct bnxt *bp,
				  uint16_t fid,
				  uint64_t *dropped);
...
int bnxt_hwrm_ctx_qstats(struct bnxt *bp,
			 uint32_t cid,
			 int idx,
			 struct rte_eth_stats *stats,
			 uint8_t rx);
...
int bnxt_hwrm_vnic_qcfg(struct bnxt *bp,
			struct bnxt_vnic_info *vnic,
			int16_t fw_vf_id);
...
int bnxt_set_hwrm_vnic_filters(struct bnxt *bp,
			       struct bnxt_vnic_info *vnic);

int bnxt_clear_hwrm_vnic_filters(struct bnxt *bp,
				 struct bnxt_vnic_info *vnic);
...
int bnxt_get_hwrm_link_config(struct bnxt *bp,
			      struct rte_eth_link *link);

int bnxt_set_hwrm_link_config(struct bnxt *bp,
			      bool link_up);
...
int bnxt_hwrm_allocate_vfs(struct bnxt *bp,
			   int num_vfs);

int bnxt_hwrm_func_vf_mac(struct bnxt *bp,
			  uint16_t vf,
			  const uint8_t *mac_addr);

int bnxt_hwrm_pf_evb_mode(struct bnxt *bp);
int bnxt_hwrm_func_bw_cfg(struct bnxt *bp,
			  uint16_t vf,
			  uint16_t max_bw,
			  uint16_t enables);
...
int bnxt_hwrm_func_qcfg_vf_default_mac(struct bnxt *bp,
				       uint16_t vf,
				       struct ether_addr *mac);

int bnxt_hwrm_func_qcfg_current_vf_vlan(struct bnxt *bp, int vf);

int bnxt_hwrm_tunnel_dst_port_alloc(struct bnxt *bp,
				    uint16_t port,
				    uint8_t tunnel_type);

int bnxt_hwrm_tunnel_dst_port_free(struct bnxt *bp,
				   uint16_t port,
				   uint8_t tunnel_type);

void bnxt_free_tunnel_ports(struct bnxt *bp);

int bnxt_hwrm_set_default_vlan(struct bnxt *bp,
			       int vf,
			       uint8_t is_vf);
...
int bnxt_hwrm_port_led_cfg(struct bnxt *bp,
			   bool led_on);

int bnxt_hwrm_port_led_qcaps(struct bnxt *bp);

int bnxt_hwrm_func_cfg_vf_set_flags(struct bnxt *bp,
				    uint16_t vf,
				    uint32_t flags);

void vf_vnic_set_rxmask_cb(struct bnxt_vnic_info *vnic,
			   void *flagp);

int bnxt_set_rx_mask_no_vlan(struct bnxt *bp,
			     struct bnxt_vnic_info *vnic);

int bnxt_vf_vnic_count(struct bnxt *bp, uint16_t vf);

int bnxt_hwrm_func_vf_vnic_query_and_config(struct bnxt *bp,
					    uint16_t vf,
					    void (*vnic_cb)
...
int bnxt_hwrm_func_cfg_vf_set_vlan_anti_spoof(struct bnxt *bp,
					      uint16_t vf,
					      bool on);
...
int bnxt_get_nvram_directory(struct bnxt *bp, uint32_t len,
			     uint8_t *data);

int bnxt_hwrm_nvm_get_dir_info(struct bnxt *bp,
			       uint32_t *entries,
			       uint32_t *length);

int bnxt_hwrm_get_nvram_item(struct bnxt *bp,
			     uint32_t index,
			     uint32_t offset,
			     uint32_t length,
			     uint8_t *data);
...
int bnxt_hwrm_flash_nvram(struct bnxt *bp,
			  uint16_t dir_type,
