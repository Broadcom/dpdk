...
#define BROADCOM_DEV_ID_NETXTREME_S_VF 0xd800
...
static const struct rte_pci_id bnxt_pci_id_map[] = {
	...
	{ RTE_PCI_DEVICE(PCI_VENDOR_ID_BROADCOM,
			 BROADCOM_DEV_ID_NETXTREME_S_VF) },
				 ...
				 struct rte_eth_dev_info *dev_info)
{
	...
	max_rx_rings = RTE_MIN(bp->max_vnics,
...
static int bnxt_mtu_set_op(struct rte_eth_dev *eth_dev, uint16_t new_mtu)
{
	...
	eth_dev->data->dev_conf.rxmode.max_rx_pkt_len =
		new_mtu + ETHER_HDR_LEN + ETHER_CRC_LEN +
		VLAN_TAG_SIZE * BNXT_NUM_VLANS;
	...
	while (raw_cons < rxq->nb_rx_desc) {
		...
		if (cmp_type == RX_TPA_END_CMPL_TYPE_RX_TPA_END) {
			cmp = (rte_le_to_cpu_32
				(((struct rx_tpa_end_cmpl *)(rxcmp))->agg_bufs_v1)
...
static bool bnxt_vf_pciid(uint16_t id)
{
	if (id == BROADCOM_DEV_ID_57304_VF ||
	    ...
	    id == BROADCOM_DEV_ID_NETXTREME_S_VF)
		return true;
	return false;
}
